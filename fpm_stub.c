...
int create_listen_sock(int port, int *sock_p)
{
	...
	if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &reuse, sizeof(reuse)) <
	    0) {
		warn_msg("Failed to set reuse addr option: %s",
			 strerror(errno));
	}
...
fpm_msg_hdr_t *read_fpm_msg(char *buf, size_t buf_len)
{
	...
	while (1) {
		...
		if (bytes_read <= 0) {
			err_msg("Error reading from socket: %s",
				strerror(errno));
			return NULL;
		}
 ...
 */
const char *netlink_msg_type_to_s(uint16_t type)
{
	switch (type) {
 ...
 * netlink_prot_to_s
 */
const char *netlink_prot_to_s(unsigned char prot)
{
	switch (prot) {
 ...
 */
static inline void netlink_msg_ctx_init(netlink_msg_ctx_t *ctx)
{
 ...
 */
static inline void netlink_msg_ctx_set_err(netlink_msg_ctx_t *ctx,
					   const char *err_msg)
{
	if (ctx->err_msg) {
		return;
 ...
 */
static inline void netlink_msg_ctx_cleanup(netlink_msg_ctx_t *ctx)
{
	return;
}
 ...
 * parse_rtattrs_
 */
int parse_rtattrs_(struct rtattr *rta, size_t len, struct rtattr **rtas,
 ...
 * parse_rtattrs
 */
int parse_rtattrs(netlink_msg_ctx_t *ctx, struct rtattr *rta, size_t len)
{
	const char *err_msg;
 ...
 */
int parse_multipath_attr(netlink_msg_ctx_t *ctx, struct rtattr *mpath_rtattr)
{
	size_t len, attr_len;
 ...
 * parse_route_msg
 */
int parse_route_msg(netlink_msg_ctx_t *ctx)
{
	int len;
 ...
 * addr_to_s
 */
const char *addr_to_s(unsigned char family, void *addr)
{
	size_t buf_len;
	char *buf;
 ...
 */
int netlink_msg_ctx_snprint(netlink_msg_ctx_t *ctx, char *buf, size_t buf_len)
{
	struct nlmsghdr *hdr;
	struct rtmsg *rtmsg;
	netlink_nh_t *nh;
 ...
 */
void print_netlink_msg_ctx(netlink_msg_ctx_t *ctx)
{
	char buf[1024];
 ...
 * parse_netlink_msg
 */
void parse_netlink_msg(char *buf, size_t buf_len, fpm_msg_hdr_t *fpm)
{
	...
	for (; NLMSG_OK(hdr, len); hdr = NLMSG_NEXT(hdr, len)) {
		...
		case RTM_NEWROUTE:
			...
			parse_route_msg(ctx);
			if (ctx->err_msg) {
				err_msg("Error parsing route message: %s",
					ctx->err_msg);
			}
		...
		default:
			trace(1, "Ignoring unknown netlink message - Type: %d",
			      hdr->nlmsg_type);
		}
